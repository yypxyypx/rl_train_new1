...
DA3 extrinsics 为 w2c OpenCV，内部取逆得 c2w 后用于 warp。
...
def compute_feature_similarity(
    ...
    DA3 extrinsics 为 w2c，内部取逆得 c2w 用于 warp。
    """
    ...
    w2c_all = torch.from_numpy(_to_4x4(da3_data["extrinsics"]).astype(np.float32)).to(device)
...
def main():
    parser = argparse.ArgumentParser(description="DINOv2 feature similarity reward")
    parser.add_argument("--video_frames_dir", required=True)
    parser.add_argument("--da3_output", required=True, help="DA3 output .npz from step_da3")
    ...
    print(f"[step_dinov2_featup] Loaded DA3 output: depth {da3_data['depth'].shape}")
    ...
    print(f"[step_dinov2_featup] reward_feature_sim = {reward:.4f} (mode={mode})")
