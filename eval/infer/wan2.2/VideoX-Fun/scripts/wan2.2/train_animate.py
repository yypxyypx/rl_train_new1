...
def main():
    ...
    if args.use_ema:
        ...
        ema_transformer3d = Wan2_2Transformer3DModel_Animate.from_pretrained(
            os.path.join(args.pretrained_model_name_or_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
            transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs']),
        ).to(weight_dtype)

        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=Wan2_2Transformer3DModel_Animate, model_config=ema_transformer3d.config)
        ...
        if fsdp_stage != 0 or zero_stage == 3:
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    _, ema_kwargs = Wan2_2Transformer3DModel_Animate.load_config(ema_path, return_unused_kwargs=True)
                    load_model = Wan2_2Transformer3DModel_Animate.from_pretrained(
                        input_dir, subfolder="transformer_ema",
                        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs'])
                    )
                    load_model = EMAModel(load_model.parameters(), model_cls=Wan2_2Transformer3DModel_Animate, model_config=load_model.config)
    ...
    # Get the dataset
    train_dataset = VideoAnimateDataset(
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            new_examples["background_pixel_values"] = []
            # Used in Mask pixel values
            new_examples["mask"] = []
            ...
            for example in examples:
                ...
                transform_512 = transforms.Compose([
                    transforms.Resize((512, 512), interpolation=transforms.InterpolationMode.BILINEAR),  # Image.BICUBIC
                ...
                new_examples["clip_pixel_values"].append(torch.from_numpy(example["clip_pixel_values"]))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                background_pixel_values = batch["background_pixel_values"].cpu()
                background_pixel_values = rearrange(background_pixel_values, "b f c h w -> b c f h w")
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.auto_tile_batch_size and args.training_with_video_token_length and zero_stage != 3:
                    if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                        ...
                        face_pixel_values = torch.tile(face_pixel_values, (4, 1, 1, 1, 1))
                        ...
                        face_pixel_values = torch.tile(face_pixel_values, (2, 1, 1, 1, 1))
                ...
                with torch.no_grad():
                    ...
                    if rng is None:
                        refert_num = np.random.choice([0, 1], p = [0.75, 0.25])
                    else:
                    ...
                    for bs_index in range(background_pixel_values.size()[0]):
                        if rng is None:
                            ...
                            zero_init_background_pixel_values_conv_in = rng.choice([0, 1], p = [0.90, 0.10])

                        if zero_init_background_pixel_values_conv_in:
                            background_pixel_values[bs_index] = background_pixel_values[bs_index] * 0
                            mask[bs_index] = torch.ones_like(mask[bs_index])
                    background_latents = _batch_encode_vae(background_pixel_values)
                    ...
                    y_reft = torch.concat([mask, background_latents], dim=1).to(device=accelerator.device, dtype=weight_dtype)

                    mask_ref    = get_i2v_mask(1, latents.size(-2), latents.size(-1), 1, device=accelerator.device)
                ...
                noise = torch.randn(
                    latents.size(), device=latents.device, generator=torch_rng, dtype=weight_dtype
                )
