...
class WanT5EncoderModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            T5SelfAttention(dim, dim_attn, dim_ffn, num_heads, num_buckets, shared_pos, dropout_p) for _ in range(num_layers)
        ])
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_path, additional_kwargs={}, low_cpu_mem_usage=False, torch_dtype=torch.bfloat16, **kwargs):
        ...
        if os.path.isfile(os.path.join(pretrained_model_path, 'config.json')):
            ...
            # Filter config_kwargs to only include valid parameters
            config_kwargs = filter_kwargs(cls, config_kwargs)
        ...
        if low_cpu_mem_usage:
            try:
                ...
                state_dict = _load_state_dict_any(pretrained_model_path)
