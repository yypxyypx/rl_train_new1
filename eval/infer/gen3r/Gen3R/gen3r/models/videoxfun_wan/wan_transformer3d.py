...
try:
    ...
    if f"{major}.{minor}" == "8.0":
        from sageattention_sm80 import sageattn  # type: ignore
        SAGE_ATTENTION_AVAILABLE = True
    elif f"{major}.{minor}" == "8.6":
        from sageattention_sm86 import sageattn  # type: ignore
        SAGE_ATTENTION_AVAILABLE = True
    elif f"{major}.{minor}" == "8.9":
        from sageattention_sm89 import sageattn  # type: ignore
        SAGE_ATTENTION_AVAILABLE = True
    elif major>=9:
        from sageattention_sm90 import sageattn  # type: ignore
        SAGE_ATTENTION_AVAILABLE = True
except:
    try:
...
@amp.autocast(enabled=False)
def rope_apply_concatenated(x, grid_sizes, freqs):
    n, c = x.size(2), x.size(3) // 2
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        freqs_i = torch.cat([
            freqs[0][:f].view(f, 1, 1, -1).expand(f, h, w, -1),
            freqs[1][:h].view(1, h, 1, -1).expand(f, h, w, -1),
            freqs[2][:true_w].view(1, 1, true_w, -1).expand(f, h, true_w, -1).repeat(1, 1, 2, 1)
        ], dim=-1).reshape(seq_len, 1, -1)
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, dtype):
        ...
        if grid_sizes[0, 1] != grid_sizes[0, 2]:
            rope_fn = rope_apply_concatenated
        else:
            rope_fn = rope_apply
        x = attention(
            q=rope_fn(q, grid_sizes, freqs).to(dtype),
            k=rope_fn(k, grid_sizes, freqs).to(dtype),
            v=v.to(dtype),
            k_lens=seq_lens,
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        # embeddings
        self.patch_embedding = nn.Conv3d(  # reduce the spatial dimension
        ...
        if add_control_adapter:
            ...
            self.control_adapter = SimpleAdapter(in_dim_control_adapter, dim, kernel_size=(2, 2), stride=(2, 2))
        else:
    ...
    def forward(
        ...
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x])  # F, H, W
        ...
        if self.teacache is not None:
            ...
            # gradient_checkpointing 可以是 bool（全开/全关）也可以是 list[bool]
            # （per-block mask，由 selective_enable_ckpt 设置，长度需 == len(blocks)）。
            _gc_cfg = self.gradient_checkpointing
            _gc_is_list = isinstance(_gc_cfg, (list, tuple))
            # 一次性 debug 打印（首次 forward 时）
            if not getattr(self, "_gc_debug_printed", False):
                if _gc_is_list:
                    n_on = sum(1 for v in _gc_cfg if v)
                    print(f"[wan_transformer3d.forward] gradient_checkpointing = list "
                          f"(len={len(_gc_cfg)}, on={n_on}, blocks={len(self.blocks)}, "
                          f"grad_enabled={torch.is_grad_enabled()})", flush=True)
                else:
                    print(f"[wan_transformer3d.forward] gradient_checkpointing = "
                          f"{_gc_cfg!r} (bool, blocks={len(self.blocks)}, "
                          f"grad_enabled={torch.is_grad_enabled()})", flush=True)
                self._gc_debug_printed = True
            for _bi, block in enumerate(self.blocks):
                _use_ckpt = (
                    bool(_gc_cfg[_bi]) if _gc_is_list and _bi < len(_gc_cfg)
                    else (False if _gc_is_list else bool(_gc_cfg))
                )
                if torch.is_grad_enabled() and _use_ckpt:
        ...
        # head
        x = self.head(x, e)  # [B, seq_len, patch_size[1]*patch_size[2]*out_dim]

        # unpatchify
        x = self.unpatchify(x, grid_sizes)
        x = torch.stack(x)  # [B, C, F, H, W]
        if self.teacache is not None:
            self.teacache.cnt += 1
    ...
    def from_pretrained(
        ...
        low_cpu_mem_usage=False, torch_dtype=torch.bfloat16, **kwargs
    ):
        ...
        if model.state_dict()['patch_embedding.weight'].size(1) != state_dict['patch_embedding.weight'].size(1):
            try:
